...
<xsl:stylesheet version="2.0"
	...
	<xsl:template name="writeCssFile">
		<xsl:result-document href="{testng:absolutePath('style.css')}"
			format="text">
			<xsl:choose>
				<xsl:when test="testng:isFilterSelected('CONF') = 'false'">
		...
		<xsl:result-document href="{testng:absolutePath('main.js')}"
            ...
            function testMethodsFilterChanged(filterCheckBox, status) {
                ...
                if (filterCheckBox != filterAll) {
                    ...
                    checkMainFilter(filterAll, filterFail, filterPass, filterSkip);
                } else {
                    filterFail.checked = filterPass.checked = filterSkip.checked = filterAll.checked;
            ...
            function checkMainFilter(filterAll, filterFail, filterPass, filterSkip) {
		...
		<xsl:result-document href="{testng:absolutePath('navigation.html')}"
			...
			<html xmlns="http://www.w3.org/1999/xhtml">
				...
				<body>
					...
					<div>
						<b>
							<a href="http://www.chinta-tms.com/Error_Screenshots.zip"
								style="color: #FF0000"> Error Screenshots</a>
						</b>
					</div>
					<div>
						<xsl:for-each select="$suiteElements">
	...
	<xsl:template name="testMethods">
		...
		<br />
		<!-- <label for="methodsFilter_CONF"> <input id="methodsFilter_CONF" type="checkbox" 
			onclick="testMethodsFilterChanged(this, 'CONF')"> <xsl:if test="testng:isFilterSelected('CONF') 
			= 'false'"> <xsl:attribute name="checked" select="false"/> </xsl:if> </input> 
			Config </label> -->
